!(function (a, b, c, d) {
  ...
  function g (a) {
    ...
    const f = a
    for (b in f) {
      if (((d = f[b]), typeof e.style[d] !== 'undefined')) { return (e = null), [d, b] }
    }
    return [!1]
  }
  function h () {
  ...
  (e.Pipe = [
    ...
    },
    {
      ...
      run () {
        const a = this._clones
        const b = this.$stage.children('.cloned')
    ...
    },
    {
      ...
      run () {
        let a
        let b
        const c = this._clones
        const d = this._items
        const e = this.settings.loop
    ...
    },
    {
      ...
      run () {
        ...
        let b
        let c
        const d = this.settings.rtl ? 1 : -1
        const e = (this.width() / this.settings.items).toFixed(3)
        let f = 0
        for (
          this._coordinates = [],
          b = 0,
    ...
    },
    {
      ...
      run () {
        let b
        let c
        const d = (this.width() / this.settings.items).toFixed(3)
        let e = {
          width:
  ...
  (e.prototype.normalize = function (b, c) {
    const e = c ? this._items.length : this._items.length + this._clones.length
  ...
  (e.prototype.to = function (c, d) {
    if (this.settings.loop) {
      const e = c - this.relative(this.current())
      let f = this.current()
      const g = this.current()
      const h = this.current() + e
      const i = g - h < 0 ? !0 : !1
      const j = this._clones.length + this._items.length
  ...
  (e.prototype.trigger = function (b, c, d) {
    const e = { item: { count: this._items.length, index: this.current() } }
    const f = a.camelCase(
      a
...
(function (a, b, c) {
  ...
  (d.prototype.fetch = function (a, b) {
    let c = a.attr('data-vimeo-id') ? 'vimeo' : 'youtube'
    let d = a.attr('data-vimeo-id') || a.attr('data-youtube-id')
    ...
    if (
      ...
      if (!(d[3].includes('vimeo'))) { throw new Error('Video URL not supported.') }
      c = 'vimeo'
    }
    ;(d = d[6]),
  ...
  (d.prototype.play = function (b) {
    ...
    let c
    let d
    const e = a(b.target || b.srcElement)
    const f = e.closest('.' + this._core.settings.itemClass)
    const g = this._videos[f.attr('data-video')]
    const h = g.width || '100%'
    const i = g.height || this._core.$stage.height()
    g.type === 'youtube'
      ? (c =
              '<iframe width="' +
              h +
...
(function (a) {
    ...
    (this._handlers = {
      ...
      'change.owl.carousel': a.proxy(function (a) {
        if (
          ...
          const b = this._core.current()
          const c = this._core.maximum()
          const d = this._core.minimum()
          a.data =
                a.property.value > c
                  ? b >= c
                    ? d
  ...
  (b.prototype.initialize = function () {
    ...
    this._controls.$indicators.on(
      ...
      'div',
      a.proxy(function (b) {
        const c = a(b.target).parent().is(this._controls.$indicators)
          ? a(b.target).index()
  ...
  (b.prototype.update = function () {
    ...
    let c
    const d = this._core.settings
    const e = this._core.clones().length / 2
    const f = e + this._core.items().length
    const g = d.center || d.autoWidth || d.dotData ? 1 : d.dotsEach || d.items
    if (
    ...
    if (
      ...
      if (
        ...
        b > 0
          ...
          : b < 0 && this._controls.$indicators.children().slice(b).remove()
      }
...
(function (a, b) {
  ...
  var c = function (d) {
    ...
    a(b).on(
      ...
      a.proxy(function () {
        const a = b.location.hash.substring(1)
        const c = this._core.$stage.children()
        const d = (this._hashes[a] && c.index(this._hashes[a])) || 0
